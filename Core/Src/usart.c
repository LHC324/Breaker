...
#if (USING_DMA == 1)
UART_DMA_BLOCK Uart_Dma __attribute__ ((at(0x24000100)));
#endif
/* USER CODE END 0 */
...
void DmaPrintf(const char *format, ...)
{
  ...
  len = vsnprintf((char *)Uart_Dma.tx_buffer, TX_BUF_SIZE + 1, format, args);
  va_end(args);
  ...
  if (HAL_UART_Transmit_DMA(&huart1, Uart_Dma.tx_buffer, len) != HAL_OK)
  {
    Error_Handler();
  }
